...
def load_tf_features(args, tokenizer, mode):
    processor = processors[args.task](args)
    features = load_raw_features(args, mode, processor, tokenizer)
    all_input_ids = np.array([f.input_ids for f in features], dtype=np.int32)
    all_attention_mask = np.array([f.attention_mask for f in features], dtype=np.int32)
    all_token_type_ids = np.array([f.token_type_ids for f in features], dtype=np.int32)
    all_slot_labels_ids = np.array([f.slot_labels_ids for f in features])
    all_intent_label_ids = np.array([f.intent_label_id for f in features])

    return [all_input_ids, all_attention_mask, all_token_type_ids, all_slot_labels_ids, all_intent_label_ids]
...
def load_raw_features(args, mode, processor, tokenizer):
